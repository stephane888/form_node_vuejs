...
class NodeRestResource extends ResourceBase {
	...
	public static function create(ContainerInterface $container, array $configuration, $plugin_id, $plugin_definition){
		return new static( $configuration, $plugin_id, $plugin_definition, $container->getParameter( 'serializer.formats' ), $container->get( 'logger.factory' )
			->get( 'form_node_vuejs' ), $container->get( 'current_user' ) );
	}
	...
	public function get($action, $id, Request $request){
		...
		if(! $this->permission->readDatas( $this->currentUser )
			->isAllowed()){
		...
		elseif($action == 'generate-user'){
			$result = $this->getDefinitionsNode( $id, 'user' );
		}
	 ...
	 * @return array[]
	 */
	public function getDefinitionsNode(string $bundle, $entity_type = 'node', $form_mode = "default"){
		
		//
		$defautForm = \Drupal::entityTypeManager()->getStorage( 'entity_view_display' )
		...
		$fields = [];
		foreach( $defautForm['content'] as $key => $field ){
			$fields[$key] = $field;
			$fields[$key]['name'] = $key;
			if(! empty( $Allfields[$key] )){
				$fieldObject = $Allfields[$key];
				$fields[$key]['label_value'] = $fieldObject->getLabel();
				$fields[$key]['description'] = $fieldObject->getDescription();
				$fields[$key]['cardinality'] = (method_exists( $fieldObject, 'getFieldStorageDefinition' )) ? $fieldObject->getFieldStorageDefinition()
					->getCardinality() : 1;
				$fields[$key]['settings'] += $fieldObject->getSettings();
			}
			...
			// 'cardinality'=> (method_exists( $field, 'getFieldStorageDefinition' )) ? $field->getFieldStorageDefinition()
			// ->getCardinality() : 1,
		...
		return [
				'fields'=> $fields,
				'bundles_infos'=> \Drupal::service( "entity_type.bundle.info" )->getAllBundleInfo()[$entity_type],
	...
	protected function getNode(int $nid){
		...
		foreach( $node->getFields() as $key => $field ){
			$fields[$key] = [
					'name'=> $field->getFieldDefinition()
						->getName(),
					'type'=> $field->getFieldDefinition()
						->getType(),
					'defaults'=> $field->getFieldDefinition()
						->getDefaultValueLiteral(),
					'require'=> $field->getFieldDefinition()
						->isRequired(),
					// 'multiple'=>$field->getFieldDefinition()->isList(),
					'settings'=> $field->getFieldDefinition()
						->getSettings(),
					'label'=> $field->getFieldDefinition()
						->getLabel(),
					...
					'description'=> (method_exists( $field->getFieldDefinition(), 'getDescription' )) ? $field->getFieldDefinition()
		...
		foreach( $fields as $k => $f ){
			...
			if(isset( $f['settings']['handler'] ) && $f['settings']['handler'] == "default:node" && ! empty( $f['valeur'] )){
				$new_entity = \Drupal::entityTypeManager()->getStorage( 'node' )
					->load( $f['valeur'][0]['target_id'] );
				$fields[$k]['infosplus'] = [
			...
			elseif(isset( $f['settings']['handler'] ) && $f['settings']['handler'] == "default:taxonomy_term" && ! empty( $f['valeur'] )){
				$new_entity = \Drupal::entityTypeManager()->getStorage( 'taxonomy_term' )
					->load( $f['valeur'][0]['target_id'] );
				$fields[$k]['infosplus'] = [
		...
		$fields['uid']['valeur'][0]['value'] = $node->getRevisionAuthor()
			->getUsername();
		//
		if(! $this->is_administrator){
			unset( $fields['status'] );
	...
	protected function getListeTerm($vocabulaire_name, $search, $limit = 15){
		...
		// /
		if(! empty( $tids )){
			$termes = \Drupal::entityTypeManager()->getStorage( 'taxonomy_term' )
				->loadMultiple( $tids );
